...
class EcoTakeCommand : CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        if(sender is Player){
            val p: Player = sender
            if(p.hasPermission("lambda.cmd.eco.take")){
                if(args.size == 1){
                    var money: Double = 0.0
                    try {
                        money = args[0].toDouble()
                    }catch(e: NumberFormatException){
                        MessageManager.sendPlayerError("Please use /ecotake money player", p)
                        return false;
                    }

                    if(LambdaEconomy.ecoCore.setPlayer(p, LambdaEconomy.ecoCore.getBalance(p) - money)) {
                        MessageManager.sendPlayerGood("You took yourself${money}${LambdaEconomy.ecoCore.currencySign()}", p)
                    ...
                    if(target != null && LambdaEconomy.ecoCore.setPlayer(target, LambdaEconomy.ecoCore.getBalance(target) - money)) {
                        MessageManager.sendPlayerGood("You took ${target.name} ${money}${LambdaEconomy.ecoCore.currencySign()}", p)
                ...
                }else MessageManager.sendPlayerError("Please use /ecotake money player", p)
